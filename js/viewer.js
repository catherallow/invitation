...
function paintBlock(blockSelector,color,type) {
    ...
    if (type == 2) {
        ...
        iframe.contents().find(blockSelector+' *:not(.sm_colors):not(.sm_color > div):not(.sm-btn):not(.sm-button):not(.sm-button-center):not(.sm_colors *):not(.slick-initialized.slick-slider *):not([class*="__bg"]):not(video):not(.text-no-paint)').css('cssText', 'color:' + color + ' !important;filter:none;-webkit-filter: none;');
        ...
        iframe.contents().find(blockSelector).find('svg:not(.text-no-paint) path').css('cssText', 'fill:' + color + ' !important;stroke:' + color + ' !important;');
        ...
        if(iframe.contents().find(blockSelector+' .text-paint-text').length>0){
            updateStyleRule(iframe.contents().find('head').find('.colors-styles'), blockSelector+' .text-paint-text', 'border-color:' + color + '!important');
        }
    ...
    $.each(data_value, function (ik, iv) {
        ...
        if ($.inArray(ik, galleries) >= 0) { //Картинки
            ...
            $.each(iv, function (ko, vo) {
                photos += '<img src="' + vo + '" data-fancybox="' + ik + '">';
            })
            iv = photos
            ...
            $.each(iv, function (ko, vo) {
                ...
                iframe.contents().find('[data-sm-src="' + ik.toUpperCase() + '_' + ko + '"]').prop('src', vo);
                iframe.contents().find('[data-sm-href="' + ik.toUpperCase() + '_' + ko + '"]').prop('href', vo);
            })
        }
        ...
        if (data_value[add + 'OWN_AFTER'] && data_value[add + 'OWN_AFTER'] != '0') {
            ...
            if (data_value[add + 'OWN_IMAGES'].length > 0) {
                $.each(data_value[add + 'OWN_IMAGES'], function (key, val) {
                    own_images += '<img src="/' + val + '" data-sm-src="' + add + 'OWN_IMAGES_' + key + '">';
                })
            }
...
function initTimingIcons() {
    ...
    $.getJSON(template_val.folder + '/config.json', function (data) {
        if (data) {
